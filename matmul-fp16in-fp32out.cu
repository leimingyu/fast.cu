	...
	uint32_t *C)	
{
	...
	wgmma_m64n8k16_f32_f16_f16<1,1,1,0,0>(descA, descB, c0, c1, c2, c3);
...
int main(int argc, char **argv)
{
	...
	std::vector<uint32_t> allTests_c;               // input c in f32 (changed from uint16_t)
	...
	while (getline(file, line))
	{
		...
		// read c first
		iss >> hexStr;
		uint32_t num_c = static_cast<uint32_t>(std::stoul(hexStr, nullptr, 16)); // Changed from uint16_t to uint32_t
    ...
    // results in fp32 (changed from fp16)
			 ...
			 std::vector<uint32_t> &current_result)
{
	...
	// Use the 32-bit C value directly
	hD[0] = current_test_c;

#if DEBUG
    printf("Input C (32-bit): %08X \n\n", hD[0]);
#endif
	...
	// Don't extract just the lower 16 bits, use the full 32-bit value
	
#if DEBUG
    printf("%08X\n", c0);
#endif

	current_result.push_back(c0);  // Push the full 32-bit result
